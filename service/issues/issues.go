...
func (u *issueController) List(id int64, param IssueParam, output *[]*IssueOutput) error {
	query := `SELECT * FROM issues WHERE time = $1 ORDER BY finish_if = false DESC ,user_id = $2 DESC `
...
func (u *issueController) Update(id int64, input IssueInput) error {
	...
	query := `UPDATE issues SET (name,level,update_at) = ($1,$2,current_timestamp) WHERE id = $3 AND finish_if = false;`
...
func (u *issueController) Delete(id int64, param IssueInput) error {
	...
	query := `DELETE FROM issues WHERE id = $1 and user_id = $2;`
